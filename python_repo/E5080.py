...
class VNA(object):
    ...
    def trace_read(self, trace=1):
        
        resp = self.v.ask("CALC1:MEAS{}:DATA:FDATA?".format(trace))
        return resp.split(",")
    ...
    def Format(self, Format='', Trace=1):
        ...
        self.v.write('CALC1:MEAS{}:FORM {}'.format(Trace, Format))
    ...
    def collect_single(self,f_range,npoints=1601,navg = 999,power=-50,wait=1,Spar = 'S21',Format='MLOG',BW=1e3,Temp_reading=True):
    ...
    def meas_complex_ave(self, f_range, npoints=1601, navg=100, power=-50,
        ...
        if navg == 0:
            ...
            self.v.write(':SENS1:AVER ON')
            self.average_count(navg)

        # Set device to external Control
        #self.v.write(':TRIG:SEQ:SOUR BUS')
        self.v.write('INIT:CONT OFF')
        self.v.write("SENS1:AVER:CLE")
        
        # Turn of delay    
        self.v.write("CALC1:MEAS1:CORR:EDEL 0")
        self.v.write("CALC1:MEAS2:CORR:EDEL 0")
        ...
        self.v.ask("*OPC?")
        # Acquire data
        freqs = np.linspace(float(self.freq_start()), float(self.freq_stop()),
                            npoints)
        x = np.asarray(freqs, dtype=np.float)
        re = np.asarray(self.trace_read(1), dtype=np.float)
        im = np.asarray(self.trace_read(2), dtype=np.float)
