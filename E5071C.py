...
class E5071C(object):
    ...
    def __init__(self, ip='192.168.187.123', *pars, **kwargs):
    ...
    def com(self, command, arg="?", raw=False):
        ...
        Use raw = True for binary data
        """
        if arg == "?":
            if raw:
                # Acquire data (follows standard device communcation:
                # #<No. digits><Amount of bytes><Data1>....
                self._inst.write('format:data Real')
                self._inst.write('{}?'.format(command))

                # Receive data (assume data is always smaller than
                # 100*20 kilobytes)
                resp = self._inst.read_raw(100)

                n_digits = int(resp[1:2])  # digits for number of bytes
                n_bytes = int(resp[2:2 + n_digits])
                n_points = n_bytes / 4 / 2  # 4 bytes is a float, 8 would be
                # double. Further divided by 2 because sdata gives always
                # real and imaginary for each point

                # Create data (big endian double >d
                data = []
                for p in struct.iter_unpack(">d", resp[2 + n_digits:-1]):
                    data.append(p[0])

                self.com('format:data', 'ASC')  # Switch back to ascii
    ...
    # READING data
    def freq_read(self):
        com_str = 'CALC:SEL:DATA:XAXis'
        return self.com(com_str, raw=True)

    def trace_read(self, trace=''):
        com_str = 'CALC:TRACe{}:DATA:FDATa'.format(trace)
        dat = self.com(com_str, raw=True)
    ...
    def get_error(self):
        resp = self._inst.query('system:error?')
        resp = resp.split('"')
        return resp[-2]
    ...
    def meas_complex(self, f_range, npoints=1601, navg=100, power=-50,
        ...
        if navg > 1:
            for i in tqdm_notebook(range(navg), unit="Sweep", leave=False):
                # Break if measurement finished
                stb = int(self.com('*ESR'))
                bDone = (stb & 1) > 0
                if bDone:
                    break
